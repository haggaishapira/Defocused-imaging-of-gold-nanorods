function [selection,value] = my_lst_dlg(varargin)
...
ex = get(0,'DefaultUicontrolFontSize')*1.7 + 30;  % height extent per line of uicontrol text (approx)

fp = get(0,'DefaultFigurePosition');
w = 2*(fus+ffs)+listsize(1) * size_factor;
h = 2*ffs+6*fus+ex*length(promptstring)+listsize(2)+uh+(smode==2)*(fus+uh) * size_factor;
...
listsize(1) = listsize(1) * size_factor;
