function best_ROI = search_ROI_3(handles,frame,ROI,patterns)
...
%     patterns = handles.partial_patterns;
    [~,min_lst_sq] = fit_pattern_least_squares(im(:),patterns);
...
%                             disp(theta);
%                             disp(phi);
                        [~,lst_sq] = fit_pattern_least_squares(im(:),patterns);
                        if lst_sq < min_lst_sq
