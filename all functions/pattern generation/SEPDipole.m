function [intx, inty, intz, rho, phi, fxx0, fxx2, fxz, byx0, byx2, byz] = SEPDipole(rho, z, NA, n1, n, n2, d1, d, d2, lambda, mag, focpos, atf, ring, block, orient)
...
% block    - beam block radius in the emission pathway for dark field
%            microscopy (mm)
...
% chimin = pi/6;
if nargin<15||isempty(block)
    chimin = 0;
else
    chimin = atan(block*mag/180)/mag/ni;
end
...
if nargin>12 && ~isempty(atf)
    if length(atf)==1 % accounting for reflection losses when using water immersion; atf = ref index of cover slide
        [~, ~, tmp, tms] = FresnelL(n1(1)*c0,n1(1),atf);
        [~, ~, mp, ms] = FresnelL(sqrt(atf^2-n1(1)^2*s0.^2),atf,n1(1));
    else % + aberration for water immersion; atf(2) = thickness mismatch
        if 1
            [~, ~, tmp, tms] = FresnelL(n1(1)*c0,[n1(1) atf(1) atf(1)], 2*pi*atf(2)/lambda);
            [~, ~, mp, ms] = FresnelL(sqrt(atf(1)^2-n1(1)^2*s0.^2),[atf(1) n1(1) n1(1)], -2*pi*atf(2)/lambda);
        else
            w0 = n1(1)*c0;
...
if nargin>15 && ~isempty(orient) % image of a molecule with orientation orient
    phi = phi - orient(2);
